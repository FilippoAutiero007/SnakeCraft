...
import { BOSS_SPAWN_SCORE, GROWTH_THRESHOLD, getLevelConfig } from '../../constants';
import { generateBlock } from '../logic';
import { audio } from '../audio';
import { processTutorialStep } from './tutorial';
import { updateBossBehavior, manageProjectiles, manageAoE } from './entities';
import { handleCollisions } from './collision';
import { manageWorld, spawnItemNearPlayer } from './world';
import { updateParticles, createExplosion, triggerHaptic } from '../effects';
...
export const updateGame = (state: any, config: any) => {
    ...
    if (!config.isTutorial && state.score.current > BOSS_SPAWN_SCORE && !state.boss.current) {
        ...
        const bossHp = levelConfig.bossHp || 200;
        state.boss.current = {
            position: { x: head.x + 10, y: head.y + 10 },
            visualPosition: { x: head.x + 10, y: head.y + 10 },
            hp: bossHp, maxHp: bossHp, isActive: true, type: levelConfig.bossType, phase: 'IDLE', attackTimer: 50
        };
    ...
    if (state.boss.current) {
        ...
        if (state.boss.current.hp <= 0) {
            createExplosion(state.boss.current.position.x, state.boss.current.position.y, 'red', 50, state.particles.current);
            triggerHaptic(200);
    ...
    updateParticles(state.particles.current);
