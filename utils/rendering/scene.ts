...
import { BlockType, Boss, Projectile, AoEZone, Particle } from '../../types';
import { BIOME_COLORS, CELL_SIZE_PX } from '../../constants';
import { getBiome, generateBlock } from '../logic';
import { drawGrassClump, drawFlowerPatch, drawPond } from './assets';
import { drawBlock } from './blocks';
import { 
  drawSnakeSegment, drawBossRealistic, drawProjectile, 
  drawAoE, drawBeams, drawStunEffect, drawTutorialTarget, drawParticles
} from './entities';
...
export const renderScene = (ctx: CanvasRenderingContext2D, data: SceneData) => {
    const { width, height, headVisual, zoom, worldMap, level, isTutorial, frame, tutorialTarget, boss, snakeVisual, currentSkin, beams, projectiles, aoeZones, particles, isBurning, isStunned } = data;
    ...
    const visibleCellsX = Math.ceil((width / zoom) / CELL_SIZE_PX / 2) + 1;
    const visibleCellsY = Math.ceil((height / zoom) / CELL_SIZE_PX / 2) + 1;
    ...
    for (let y = startY; y <= endY; y++) {
      for (let x = startX; x <= endX; x++) {
        ...
        // Decorations only every 2 frames for performance
        if (frame % 2 === 0 && biome === 'GRASSLAND' && !worldMap.has(`${x},${y}`)) {
            const rand = getPseudoRand(x, y);
            if (rand < 0.15) {
                if (rand < 0.05) drawGrassClump(ctx, x * CELL_SIZE_PX, y * CELL_SIZE_PX, CELL_SIZE_PX, frame);
                else if (rand < 0.10) drawFlowerPatch(ctx, x * CELL_SIZE_PX, y * CELL_SIZE_PX, CELL_SIZE_PX, frame);
            }
        }
      }
    }
    ...
    drawBeams(ctx, beams);
    drawParticles(ctx, particles, CELL_SIZE_PX);
